...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
    ...
    // ตั้งค่ารูปล่าสุด (ถ้าไม่ได้อัปโหลดไฟล์ จะใช้ค่าจาก preset ที่ถูกส่งมา)
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if(move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)){
            // ลบรูปเก่าทิ้ง (ถ้าไม่ใช่รูปเริ่มต้น)
    ...
    $stmt = $conn->prepare("UPDATE devices SET category_id=?, name=?, image=?, ip_address=?, port=?, slave_id=? WHERE id=?");
    $stmt->bind_param("issssii", $cat_id, $name, $final_image, $ip, $port, $slave_id, $id);
