...
if(!$device) {
    echo "<script>alert('ไม่พบอุปกรณ์นี้'); window.location.href='devices.php';</script>";
    exit;
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style> 
        ...
        .console { font-family: 'JetBrains Mono', monospace; background: #111827; color: #10b981; padding: 15px; border-radius: 8px; min-height: 300px; font-size: 0.9rem;} 
        .status-dot { display: inline-block; width: 10px; height: 10px; border-radius: 50%; margin-right: 5px; }
        .dot-active { background-color: #10b981; box-shadow: 0 0 8px #10b981; animation: pulse 1.5s infinite; }
...
<div class="main-content p-4">
    <div class="container-fluid">
        <div class="d-flex align-items-center justify-content-between mb-4">
            ...
            <button onclick="deleteDevice(<?= $device['id'] ?>, '<?= htmlspecialchars($device['name']) ?>')" class="btn btn-danger shadow-sm"><i data-lucide="trash-2" class="w-4 h-4 inline-block me-1"></i> ลบอุปกรณ์นี้</button>
        </div>
...
<script>
    ...
    async function readModbusData() {
        ...
        try {
            ...
            if(data.status === 'success') {
                statusConsole.innerHTML = `> Status: <span style="color:#10b981">OK (Code 200)</span> | Byte Count: ${data.byte_count} bytes`;
                ...
                data.registers.forEach((val, index) => {
                    ...
                    output += `<tr><td>${startAddr + index}</td><td class="fw-bold">${val}</td><td>${hex}</td></tr>`;
                });
                output += `</tbody></table>`;
                
                dataConsole.innerHTML = output;
            } else {
                // 🚨 แสดง Error แบบโดดเด่นสีแดง
                statusConsole.innerHTML = `> 🚨 <span style="color:#ef4444; font-weight:bold;">${data.message}</span>`;
                dataConsole.innerHTML = '';
            }
        } catch (err) {
            statusConsole.innerHTML = `> 🚨 <span style="color:#ef4444; font-weight:bold;">System Error: Cannot reach API Server</span>`;
            dataConsole.innerHTML = '';
        }
    }
    ...
    // 🚨 ระบบลบอุปกรณ์สำหรับหน้านี้
    function deleteDevice(id, name) {
        Swal.fire({
            title: 'ยืนยันการลบอุปกรณ์?',
            html: `คุณกำลังจะลบ <b>${name}</b><br><span style="color:red; font-size: 0.9em;">*ข้อมูลประวัติจะหายทั้งหมด!</span>`,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                fetch('api/delete_device.php', {
                    ...
                    if(data.status === 'success') {
                        Swal.fire('สำเร็จ!', 'อุปกรณ์ถูกลบแล้ว', 'success').then(() => {
                            window.location.href = 'devices.php'; // เด้งกลับหน้ารวม
                        });
                    } else {
